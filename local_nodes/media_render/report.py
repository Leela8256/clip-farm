...
def cache_hit(report, cache_key: str, tier: str, window=None) -> bool:
    """
    Whether a render already on disk was made from exactly this spec: the same
    cache key, the same tier and the same output window. Anything else — an
    older report, a different tier, a report with no key — is a miss, so the
    render runs rather than handing back the wrong picture.
    """
    if not isinstance(report, dict) or not cache_key:
        return False
    if (report.get('cache_key') or report.get('spec_hash')) != cache_key:
        return False
    detail = report.get('quality')
    if not isinstance(detail, dict) or detail.get('tier') != tier:
        return False
    wanted = [int(window[0]), int(window[1])] if window else None
    return (report.get('range') or None) == wanted


def build_render_report(*, kind: str = 'render', mode: str = 'preview', quality: str = '',
                        detail: dict | None = None, check: dict, version=None, title=None,
                        measured=None, measurements=None, target_lufs=None, mastered: bool = True,
                        window=None, preview_output_start_ms: int = 0, total_ms: int = 0,
                        body_ms: int = 0, lead_ms: int = 0, tail_ms: int = 0, files=None,
                        outputs=None, extras=None, source_range=None, fps=None, cuts: int = 0,
                        mutes: int = 0, bleeps: int = 0, captions_on: bool = False,
                        caption_lines: int = 0, caption_style: dict | None = None,
                        caption_preset=None, chapters=None, music: bool = False,
                        expect_video: bool = True, parts=None, cache_key: str = '',
                        framing: dict | None = None, warnings: list[str] | None = None,
    ...
    report = {
        ...
        'layout': framing,
        'cache_key': cache_key,
        'spec_hash': cache_key,                 # schema-2 name, kept for existing readers
        'warnings': warnings,
        'validation': {
