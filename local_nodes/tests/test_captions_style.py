...
from local_nodes.podcast_common.captions import (  # noqa: E402
    CAPTION_LAYOUTS,
    build_ass,
    build_srt,
    build_vtt,
...
_LEGACY_PRESETS = {
    'classic': {'highlight': '&H004A6BFF', 'text': '&H00FFFFFF', 'outline': '&H00000000', 'back': '&H80000000',
                'bold': -1, 'outline_px': 4, 'shadow': 0, 'scale': 1.0},
    'yellow-bold': {'highlight': '&H0000D4FF', 'text': '&H00FFFFFF', 'outline': '&H00402010', 'back': '&H80000000',
                    'bold': -1, 'outline_px': 5, 'shadow': 1, 'scale': 1.12},
    'white-outline': {'highlight': '&H00FFFFFF', 'text': '&H00E6E6E6', 'outline': '&H00000000', 'back': '&HFF000000',
                      'bold': -1, 'outline_px': 5, 'shadow': 0, 'scale': 1.0},
    'minimal': {'highlight': '&H00FFFFFF', 'text': '&H00E6E6E6', 'outline': '&H00000000', 'back': '&HFF000000',
...
class LegacyPresetParityTest(unittest.TestCase):
        ...
        for preset in self.PRESETS:
            for layout in ('vertical', 'wide'):
                with self.subTest(preset=preset, layout=layout):
                    self.assertEqual(build_ass(GROUPS, layout, preset),
                                     _legacy_build_ass(GROUPS, layout, preset))

    def test_seam_placement_still_lands_on_the_same_bytes(self):
        place = seam_placement([{'start_ms': 0, 'end_ms': 1000, 'layout': 'stacked_two'}])
        self.assertEqual(build_ass(GROUPS, 'vertical', 'classic', placement=place),
    ...
    def test_a_custom_font_name_is_honoured_as_before(self):
        self.assertEqual(build_ass(GROUPS, 'wide', 'minimal', font_name='Inter'),
    ...
    def test_a_style_object_naming_only_a_legacy_preset_changes_nothing(self):
        self.assertEqual(build_ass(GROUPS, 'vertical', style={'preset': 'yellow-bold'}),
...
class ResolveStyleTest(unittest.TestCase):
    def test_a_string_becomes_the_full_object(self):
        ...
        for key in ('font', 'weight', 'size', 'case', 'color', 'outline', 'shadow', 'box', 'position',
                    'alignment', 'max_words', 'max_lines', 'emphasis', 'keywords', 'speaker_colors'):
    ...
    def test_the_nine_gallery_presets_all_resolve(self):
        for name in ('none', 'clean-karaoke', 'bold-impact', 'yellow-punch', 'white-outline',
                     'boxed-focus', 'color-pop', 'minimal', 'two-line-social'):
    ...
    def test_overrides_win_over_the_preset_and_rubbish_is_ignored(self):
        style = resolve_caption_style({'preset': 'minimal', 'color': '#123456', 'weight': 'black',
                                       'case': 'upper', 'max_words': 6, 'karaoke': False,
                                       'position': 'top', 'outline': {'width': 7}, 'font': ' Inter '})
...
class StyleToAssTest(unittest.TestCase):
    ...
    def test_weight_case_and_size_reach_the_style_line_and_the_text(self):
        ass = build_ass(GROUPS, 'vertical', style={'preset': 'classic', 'weight': 'normal', 'case': 'upper',
                                                   'size': 96})
    ...
    def test_colours_outline_and_shadow_are_converted_to_ass(self):
        ass = build_ass(GROUPS, 'vertical', style={'preset': 'classic', 'color': '#102030',
                                                   'highlight_color': '#FFD400', 'shadow': True,
    ...
    def test_position_and_alignment_pick_the_ass_alignment(self):
        for position, alignment, expected in (('bottom', 'center', '2'), ('bottom', 'left', '1'),
                                              ('middle', 'center', '5'), ('top', 'center', '8'),
                                              ('top', 'left', '7')):
            with self.subTest(position=position, alignment=alignment):
                fields = self._style_line(build_ass(GROUPS, 'vertical', style={
                    'preset': 'classic', 'position': position, 'alignment': alignment})).split(',')
    ...
    def test_speaker_colours_tint_the_line_when_the_style_asks_for_them(self):
        ...
        ass = build_ass(GROUPS, 'vertical', style={'preset': 'classic', 'speaker_colors': True},
